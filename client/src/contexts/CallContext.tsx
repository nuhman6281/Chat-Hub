...
export function CallProvider({ children }: { children: ReactNode }) {
  ...
  const { connected, on, send } = useSocket();
  ...
  }, [connected, user, activeCall, incomingCall, send, on]);
  ...
  }, [connected, user, outgoingCall, callType, on]);
  ...
  }, [connected, user, outgoingCall, toast, on]);
  ...
  }, [connected, user, activeCall, toast, on]);
  ...
  const initializeCallWithUser = async (
    userId: number,
    displayName: string,
    callType: CallType
  ) => {
    ...
    try {
      ...
      const remoteParticipant: CallParticipant = {
        userId,
        email: "", // This will be updated when we receive the user's info
        displayName,
        avatarUrl: null,
        audioEnabled: true,
  ...
  const startCall = async (userId: number, type: CallType): Promise<void> => {
    ...
    audio
      .play()
      .catch((err) => console.error("Error playing ringback tone:", err));
  ...
  const answerCall = async (): Promise<void> => {
    if (!user || !connected || !callerId) return;
    ...
    try {
      ...
      peer.on("stream", (remoteStream: MediaStream) => {
        setParticipants((prev) =>
          prev.map((p) =>
            p.userId === callerId ? { ...p, stream: remoteStream } : p
          )
        );
      });
  ...
  const endCall = () => {
    if (!connected || (!activeCall && !outgoingCall)) return;
  ...
  const toggleAudio = () => {
    ...
    setParticipants((prev) =>
      prev.map((p) =>
        p.userId === user?.id ? { ...p, audioEnabled: newState } : p
      )
    );
  ...
  const toggleVideo = () => {
    ...
    setParticipants((prev) =>
      prev.map((p) =>
        p.userId === user?.id ? { ...p, videoEnabled: newState } : p
      )
    );
